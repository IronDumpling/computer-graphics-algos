...
{
      typedef Eigen::Map< Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> > MapRXd;
      ...
      read_model_and_rig_from_json(argc>1 ? argv[1] : "../data/beast.json",
                                    V,F,skeleton,W,fk_anim,b);
      ...
      Eigen::MatrixXi SF;
      const double thickness = 0.01 * (V.colwise().maxCoeff() - V.colwise().minCoeff()).norm();
      ...
      // Update the skeleton mesh and the linear blend skinning model based on current skeleton deformation
      const auto update = [&]()
      {
        skeleton_visualization_mesh(skeleton,thickness,SV,SF,SC);
        v.data_list[skeleton_id].set_mesh(SV,SF);
        ...
        // Draw teal dots with lines attached to end-effectors and constraints (pink if selected)
      ...
      {
        if(use_ik) ik();
        else // Forward Kinematics animation
        {
